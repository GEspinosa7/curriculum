...
const createPersonaLanguage = async (req, res) => {
    ...
    try {
        ...
        if (!newLanguage[0]) return res.status(500).json({ error: error500 });

        const personaLanguages = await knex('persona_languages').insert({ languages_id: newLanguage[0].id, persona_id: personaId }).returning('*');
        if (!personaLanguages[0]) return res.status(500).json({ error: error500 });

        return res.status(201).json(personaLanguages[0]);
    } catch (error) {
...
const updatePersonaLanguage = async (req, res) => {
    ...
    try {
        ...
        if (!updatedLanguage[0]) return res.status(500).json({ error: error500 });
...
const getPersonaLanguageList = async (req, res) => {
    ...
    try {
        ...
        if (!languages) return res.status(404).json({ error: error404('Language list') });
